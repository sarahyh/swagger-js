...
var schemaToHTML = function (name, schema, models, modelPropertyMacro) {
  ...
  var primitiveToOptionsHTML = function (schema, html) {
    ...
    if (html == 'string') {
      html = createHtml(type, schema.description); 
    }

    return html;
  };
  
  var createHtml = function (type, description) {
    return '<span class="propLabels">' + 
  ...
  var processModel = function (schema, name) {
    ...
    var strongOpen = '<span class="strong objectName"><span class="bracketsIcon">' + typeStr + '</span> <span class="objectNameText">';
    ...
    if (isArray) {
      if (_.isArray(schema.items)) {
        ...
        if (name !== 'Inline Model') {
          ...
          var arrayType = 'array[' + ref + ']';
          html += '<div>' + createHtml(arrayType, schema.description) +'</div>';
          //html += '<div>' + ref + '</div>';
        }
      } else {
      ...
      if (schema.$ref) {
        ...
        if (_.isPlainObject(schema.properties)) {
          html += _.map(schema.properties, function (property, name) {
            ...
            var isDeprecated = property['x-deprecated'] && property['x-deprecated'] === true;
            if (isDeprecated) {
              html += '<span class="propName propOpt deprecated">' + name + '</span>';
            } else {
            ...
            if (isDeprecated) {
              html += '<span class="propDesc markdown deprecated">';
            } else { 
              html += '<span class="propDesc markdown">';
            }
