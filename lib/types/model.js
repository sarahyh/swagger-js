...
var schemaToHTML = function (name, schema, models, modelPropertyMacro) {
  ...
  var processModel = function (schema, name) {
    ...
    console.log("processModel " + name + " schema title: " + schema.title);
    ...
    if (isArray) {
      ...
      if (schema.$ref) {
        ...
        var primitiveTypeHtml = primitiveToOptionsHTML(schema, type);
        if (primitiveTypeHtml == 'string') {
          primitiveTypeHtml = '<span class="propLabels">' + 
          '<span class="propType" title="' + type + '">' + type + '</span></span>' +
          '<span class="propDesc">' +
          schema.description + 
          '</span>';
        }
        html += '<div>' + primitiveTypeHtml + '</div>';
        console.log("primitiveToOptionsHTML with " + type + " html " + html + " schema: " + schema);
      }
    }
...
Model.prototype.getMockSignature = function () {
  var html = schemaToHTML(this.name, this.definition, this.models, this.modelPropertyMacro);
  console.log(this.definition);
  console.log(this.name + " getMockSignature: " + html);
  return html;
};
